    ...
    <div ref={containerRef} className="min-h-screen bg-linear-to-b from-gray-50 to-white overflow-hidden">
      ...
      <section className="relative h-screen min-h-[600px] flex items-center justify-center overflow-hidden bg-black" id="Hero">
        <div className="absolute inset-0 z-0 overflow-hidden">
          {/* Enhanced Gradient Overlay for vibrancy */}
          <div className="absolute inset-0 z-10 bg-linear-to-b from-black/40 via-orange-950/20 to-black/60" />
          <motion.div style={{ y: parallaxY }} className="absolute inset-0 w-full h-full">
            <video
              className="absolute top-1/2 left-1/2 w-[300%] h-[300%] 
                     ...
                     object-cover pointer-events-none opacity-80 contrast-110 saturate-110"
              src={backgroundVideo}
              autoPlay
              loop
              muted
            ...
            {[
              ...
              <FadeInUp key={index} delay={index * 0.2}>
                <Link to={`/berita/${news.id}`}>
                  <motion.div
                    whileHover={{ y: -5 }}
                    className="bg-gray-50 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all border border-gray-100 group h-full"
                  >
                    ...
                    <div className="p-8">
                      <h3 className="text-2xl font-bold text-gray-900 mb-4 hover:text-orange-600 transition-colors cursor-pointer leading-tight line-clamp-2">
                        {news.title}
                      </h3>
                      <p className="text-gray-500 mb-6 line-clamp-2">{news.desc}</p>
                      <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                        <span className="text-orange-600 font-semibold text-sm hover:underline uppercase tracking-wider">
                          Read More
                        </span>
      ...
      <section className="py-20 bg-orange-50">
        <div className="container mx-auto px-6 text-center">
          ...
          <div className="flex flex-col md:flex-row justify-center gap-6">
            <Link
              to="/konsultasi"
              className="bg-orange-500 text-white px-8 py-4 rounded-xl font-bold hover:bg-orange-600 shadow-lg shadow-orange-500/20 text-lg flex items-center justify-center transition-all hover:scale-105"
            >
              Konsultasi Zakat
            </Link>
            <Link
              to="/kalkulator-zakat"
              className="bg-amber-500 text-white px-8 py-4 rounded-xl font-bold hover:bg-amber-600 shadow-lg shadow-amber-500/20 text-lg flex items-center justify-center transition-all hover:scale-105"
            >
              Kalkulator Zakat
            </Link>
            <Link
